...
import net.lifepost.service.model.member.SocialType;
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        OAuthAttributes extractAttributes = OAuthAttributes
            .of(socialType, userNameAttributeName, attributes);
     ...
     * saveMember()를 호출하여 회원을 저장한다.
     */
    private Member getMember(OAuthAttributes attributes, SocialType socialType) {
        Optional<Member> member = memberRepository
            .findBySocialTypeAndEmail(socialType, attributes.getEmail());

        if (member.isEmpty()) {
            return saveMember(attributes, socialType);
        }

        return member.orElseThrow(() -> new RuntimeException("getMember error."));
    }
